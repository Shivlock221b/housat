...
export async function GET(_: Request, { params }: { params: { publicToken: string } }) {
  ...
  const { data: ticket, error: ticketError } = await supabase
    .from("rental_tickets")
    .select("id,public_token,status,created_at,user_name,city,preferred_localities,budget_min,budget_max,bhk,property_types,furnishing,move_in_date,tenant_type,brokerage_preference,must_haves,nice_to_haves,deal_breakers,shortlist_ready")
